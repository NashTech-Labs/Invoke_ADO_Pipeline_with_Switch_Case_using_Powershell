...
$component_file = Get-Content -Path "<file-path>naas-component.json" | ConvertFrom-Json #URL for Component file

#pipeline
$token ="" #PAT-Token for ADO Pipeline
$organization_name=""  #provide  your organization of ADO
$base64AuthInfo = [Convert]::ToBase64String([Text.Encoding]::ASCII.GetBytes(("{0}:{1}" -f $user,$token)))
$pipeline_json = Get-Content -Path "<file-path>id.json" | ConvertFrom-Json #URL for Pipeline resource type and ID
...
foreach ($number in $test) {
    ...
    if ( $depends_On_in_file -eq $depends_On_global ) {
        ...
        if ($pipeline_resourceType -eq $type_of_resource)
        {
            ...
            #switch statement for each non-dependent resources call
            switch ($type_of_resource){
                event-hub{
                # To fetch the ID url
                $fetchPipelineId = Get-Content -Path "<file-path>id.json" | ConvertFrom-Json `
